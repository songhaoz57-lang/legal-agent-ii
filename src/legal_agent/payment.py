...
# Simple file-based paid-user store (Render ephemeral disk ok for MVP)
...
def is_session_paid(session_id: str) -> bool:
    """Check if a session ID has been paid."""
...
def create_checkout_session() -> Optional[dict]:
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            success_url=SITE_URL.rstrip("/") + "?session_id={CHECKOUT_SESSION_ID}",
            cancel_url=SITE_URL.rstrip("/"),
        )
